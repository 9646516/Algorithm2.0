...
template <typename FlowType, typename CostType> class MinCostMaxFlow {
  private:
    ...
    struct Edge {
        ...
        Edge(int to, FlowType cap, CostType cost, int rev)
  ...
  public:
    MinCostMaxFlow(int n)
        : n(n), edge(n), negative_loop(0), has_negative_cost(false), flow_s(0), flow_t(0) {}
    ...
    std::pair<FlowType, CostType> Solve(int s, int t) {
        ...
        auto relabel = [&](int x, CostType delta) { potential[x] -= delta + eps; };
    ...
    std::vector<std::pair<FlowType, CostType>> SolveSmallFlowSlope(int s, int t) {
        ...
        std::priority_queue<std::pair<CostType, int>, std::vector<std::pair<CostType, int>>,
                            std::greater<std::pair<CostType, int>>>
            q;
        std::vector<int> qmin;
